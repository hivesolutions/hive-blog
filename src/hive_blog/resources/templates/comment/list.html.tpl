<div id="comments-area">
    ${foreach item=comment index=comment_index from=post.comments}
        <div class="comment ${if item=comment.author.object_id value=post.author.object_id operator=eq}author${/if}">
            <div class="comment-number">${out value=comment_index format="%02.d" xml_escape=True /}</div>
    ...
    <div id="comments-form-area">
        ...
        <form action="${out value=base_path /}comments" id="comment-form" class="${if item=session.user_information value=None operator=eq}hidden${/if}" method="post">
            ...
            <br />
            <input type="hidden" name="comment[post][object_id]" value="${out value=post.object_id xml_escape=True /}" />
            <input type="hidden" name="return_address" value="${out value=return_address xml_escape=True /}" />
